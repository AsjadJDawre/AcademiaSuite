...
const StudentAttendence = () => {
  ...
  const getAllStudents = async () => {
    try {
      ...
      const regularStudents = fetchedStudents.filter(
  ...
  const handleSave = async () => {
    ...
    if (isEligible) {
      setShowErrorModal(true);
      setErrorMessage(
        "Students Are Already Eligible for this Exam. You can EDIT."
      );
      return;
    }
  ...
  const deleteExamAllotment = async (studentId) => {
    try {
      if (subjectType !== "ALL") {
        await window.api.invoke("delete-student-exam", {
          exam_id: examIdForeignKey,
      ...
      console.error(
        `Error deleting exam allotment for student with ID: ${studentId}`,
        error
      );
      handleRefresh();
  ...
  const handleEdit = async () => {
    ...
    try {
      const response = await window.api.invoke("fetch-student-exams", {
        exam_id: examIdForeignKey,
        semester,
      });
      ...
      const filteredStudents = response.filter(
        (student) => student.subject_marker === "ALL" || "SINGLE"
      );
      ...
      const regularStudents = allStudents.filter(
        (student) => student.status === "regular"
      );
      ...
      setEligibleStudents(
        regularStudents.filter((student) =>
          eligibleStudentIds.has(student.student_id)
        )
      );
    ...
    <>
      <div className="exam-code-container">
        ...
        <div className="student-list max-h-96 overflow-y-auto p-4">
          ...
          >
            <p>
              Are you sure you want to delete the exam allotment for this
              student?
            </p>
          </Modal>
